...
## Task 5
  ...
  Далее запускаем [скрипт (python2)](/6/script1.py), который работает  с ***dumpfile*** (имя файла упоминается в коде, поэтому если на предыдущем шаге дается другое имя файла для записи, то соотвествующее изменение нужно внести в скрипт - строка 2):
